...
{
    public class VM_DanhSachBN : BaseViewModel
    {
        ...
        private List<BENHNHAN> _allBN = new List<BENHNHAN>();

        public ObservableCollection<string> ListGioiTinhFilter { get; set; }

        private string _selectedGioiTinhFilter = "Tất cả";
        public string SelectedGioiTinhFilter
        {
            get => _selectedGioiTinhFilter;
            set { _selectedGioiTinhFilter = value; OnPropertyChanged(); ThucHienLoc(); }
        }
        ...
        {
            var filtered = _allBN.AsEnumerable();

            if (!string.IsNullOrWhiteSpace(SearchKeyword))
            {
                var kw = SearchKeyword.ToLower();
                filtered = filtered.Where(x => 
                    (x.TEN_BENHNHAN != null && x.TEN_BENHNHAN.ToLower().Contains(kw)) ||
                    (x.MA_BENHNHAN != null && x.MA_BENHNHAN.ToLower().Contains(kw))
                );
            }

            if (SelectedGioiTinhFilter != "Tất cả")
            {
                filtered = filtered.Where(x => x.GIOITINH == SelectedGioiTinhFilter);
            }

            DSBN = new ObservableCollection<BENHNHAN>(filtered.ToList());
        }
        ...
        public VM_DanhSachBN()
        {
            ListGioiTinhFilter = new ObservableCollection<string> { "Tất cả", "Nam", "Nữ", "Khác" };
            ...
            LenhXoa = new RelayCommand(p => {
                if (MessageBox.Show($"Bạn có chắc chắn muốn xóa hồ sơ: {BNDangChon.TEN_BENHNHAN}?", "Xác nhận", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                {
                    try
                    {
                        ...
                        var removedItem = _allBN.FirstOrDefault(x => x.MA_BENHNHAN == BNDangChon.MA_BENHNHAN);
                        if (removedItem != null) _allBN.Remove(removedItem);
                        ThucHienLoc();
        ...
        private void TaiDL()
        {
            ...
            try
            {
                using (var db = new QL_PHONG_KHAM())
                {
                    _allBN = db.BENHNHANs.OrderBy(x => x.MA_BENHNHAN).ToList();
                    DSBN = new ObservableCollection<BENHNHAN>(_allBN);
                }
            }
            catch (Exception ex)
            {
