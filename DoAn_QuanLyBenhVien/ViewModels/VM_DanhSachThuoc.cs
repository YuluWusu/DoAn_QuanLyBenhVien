...
{
    public class VM_DanhSachThuoc : BaseViewModel
    {
        ...
        public ObservableCollection<string> ListTinhTrangFilter { get; set; }

        private string _selectedTinhTrangFilter = "Tất cả";
        public string SelectedTinhTrangFilter
        {
            get => _selectedTinhTrangFilter;
            set { _selectedTinhTrangFilter = value; OnPropertyChanged(); ExecuteSearch(); }
        }
        ...
        {
            get => _searchKeyword;
            set { _searchKeyword = value; OnPropertyChanged(); ExecuteSearch(); }
        }
        ...
        public VM_DanhSachThuoc()
        {
            ListTinhTrangFilter = new ObservableCollection<string> { "Tất cả", "Còn hàng", "Hết hàng" };
            LoadData();
        ...
        private void ExecuteSearch()
        {
            ...
            if (SelectedTinhTrangFilter == "Còn hàng")
                filtered = filtered.Where(x => x.SOLUONG_TON > 0);
            else if (SelectedTinhTrangFilter == "Hết hàng")
                filtered = filtered.Where(x => x.SOLUONG_TON == null || x.SOLUONG_TON <= 0);

            ListThuoc = new ObservableCollection<THUOC>(filtered.ToList());
        }
    }
}
