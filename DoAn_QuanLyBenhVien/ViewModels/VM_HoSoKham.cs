...
{
    public class VM_HoSoKham : BaseViewModel
    {
        ...
        private System.Collections.Generic.List<PHIEUKHAM> _originalPhieuKham = new System.Collections.Generic.List<PHIEUKHAM>();
                    ...
                    try
                    {
                        ...
                        if (SelectedPhieuKham == null) // CHẾ ĐỘ THÊM MỚI
                        {
                            ...
                            var newPhieu = new PHIEUKHAM
                            {
                                ...
                                MANV = VM_Login.MaNVHienTai ?? "NV01",
        ...
        private void LoadData()
        {
            ...
            try
            {
                using (var db = new QL_PHONG_KHAM())
                {
                    ...
                    _originalPhieuKham = list.ToList();
        ...
        private void ExecuteSearch()
        {
            ...
            {
                DSPhieuKham = new ObservableCollection<PHIEUKHAM>(_originalPhieuKham);
            }
            else
            {
                var kw = SearchText.ToLower();
                var filtered = _originalPhieuKham.Where(x => 
                    (x.MA_PHIEUKHAM != null && x.MA_PHIEUKHAM.ToLower().Contains(kw)) ||
                    (x.BENHNHAN != null && x.BENHNHAN.TEN_BENHNHAN != null && x.BENHNHAN.TEN_BENHNHAN.ToLower().Contains(kw))
                ).ToList();
                DSPhieuKham = new ObservableCollection<PHIEUKHAM>(filtered);
            }
        }
    }
}
